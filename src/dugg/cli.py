...
def cmd_invite_user(args):
    ...
    print()
    print("--- For your AI agent ---")
    print()
    print("Hand your agent this invite link and tell it to register you both.")
    print("The agent handles the full process — no browser needed.")
    if server_url:
        invite_url = f"{server_url.rstrip('/')}/invite/{token}"
        print()
        print(f"Step 1: GET {invite_url}")
        print(f"  Header: Accept: application/json")
        print(f"  → Returns redemption instructions")
        print()
        print(f"Step 2: POST {invite_url}/redeem")
        print(f"  Header: Content-Type: application/json")
        print(f'  Body: {{"name": "{args.name}"}}')
        print(f"  → Returns your key + agent key + connection details")
        print()
        print(f"Step 3: Connect via MCP")
        print(f"  SSE endpoint: {server_url.rstrip('/')}/sse")
        print(f"  Auth header: X-Dugg-Key: <agent_api_key from step 2>")
        print()
        print(f"Step 4: Call dugg_welcome to orient")
    print()
