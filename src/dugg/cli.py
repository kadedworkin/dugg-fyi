...
def cmd_invite_user(args):
    ...
    print()
    print("Read the partner guide before connecting:")
    print("  https://github.com/kadedworkin/dugg-fyi/blob/main/PARTNER_AGENT.md")
    if server_url:
        print()
        print("Your agent can self-onboard by sending the invite link with")
        print("Accept: application/json — it returns machine-readable instructions")
        print("for redemption, MCP connection, and first steps.")
        print()
        print(f"SSE endpoint: {server_url.rstrip('/')}/sse")
        print("Auth header: X-Dugg-Key: <agent_api_key from redemption>")
    print()
    print("--- End ---")
    db.close()
