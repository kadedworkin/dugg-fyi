...
def _handle_invite_user(d: DuggDB, user_id: str, args: dict) -> list[TextContent]:
    ...
    if endpoint:
        ...
        invite_text += (
            ...
            f"\n1. Set up a local Dugg instance\n"
            f"\n   git clone https://github.com/kadedworkin/dugg-fyi.git\n"
            f"   cd dugg-fyi && uv sync\n"
            f"   dugg init\n"
            f"   dugg serve\n"
            f"\n   This is your personal knowledge base. Everything you add\n"
            f"   lives here first. You choose what to share.\n"
            f"\n2. Redeem this invite on the shared server\n"
            f"\n   GET {url}\n"
            ...
            f"\n3. Connect to the shared server via MCP\n"
            f"\n   SSE endpoint: {endpoint.rstrip('/')}/sse\n"
            f"   Auth header: X-Dugg-Key: <agent_api_key from step 2>\n"
            f"\n4. Call dugg_welcome to orient, then dugg_feed to complete onboarding\n"
            f"\n5. Publish from local to shared\n"
            f"\n   Use dugg_publish(resource_id, targets=[\"{instance_name}\"])\n"
            f"   to push selected resources from your local instance to the\n"
            f"   shared server. Your local is the source of truth — you control\n"
            f"   what gets shared.\n"
