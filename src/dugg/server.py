...
def _handle_invite_user(d: DuggDB, user_id: str, args: dict) -> list[TextContent]:
    ...
    if endpoint:
        ...
        invite_text += (
            ...
            f"\n3. Sync shared content into local\n"
            f"\n   dugg rss subscribe {endpoint.rstrip('/')}/feed/<user_api_key> --tag {instance_name.lower().replace(' ', '-')}\n"
            f"   dugg rss poll\n"
            f"\n   This backfills ALL existing content from {instance_name} into\n"
            f"   your local Dugg. No empty starting point — everything is\n"
            f"   immediately searchable locally. New items sync every hour.\n"
            f"\n4. Connect to {instance_name} via MCP\n"
            f"\n   SSE endpoint: {endpoint.rstrip('/')}/sse\n"
            f"   Auth header: X-Dugg-Key: <agent_api_key from step 2>\n"
            f"\n5. Explore what's already here\n"
