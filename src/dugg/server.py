...
def _handle_invite_user(d: DuggDB, user_id: str, args: dict) -> list[TextContent]:
    ...
    if endpoint:
        ...
        invite_text += (
            ...
            f"   dugg init\n"
            f"   dugg serve\n"
            f"\n   This is your personal knowledge base — complete on its own.\n"
            f"   Everything you add lives here first.\n"
            ...
            f"\n4. Start receiving\n"
            f"\n   Call dugg_welcome to orient yourself, then dugg_feed to see\n"
            f"   what others have shared. Use dugg_catchup to get new content\n"
            f"   since your last check. Set up dugg_webhook_subscribe to get\n"
            f"   push notifications instead of polling.\n"
            f"\n   This is where the value starts — everyone else's curated\n"
            f"   finds flow to you through the shared server.\n"
            f"\n5. Publish back\n"
            f"\n   Use dugg_publish(resource_id, targets=[\"{instance_name}\"])\n"
            f"   to share selected resources from your local. Your local is\n"
            f"   the source of truth — you control what gets shared.\n"
            f"\n   The more servers you subscribe to, the more content flows\n"
            f"   to you. Each subscription is another source of curated signal.\n"
        ...
        invite_text += (
            ...
            f"   dugg init\n"
            f"   dugg serve\n"
            f"\n   Your personal knowledge base — complete on its own.\n"
            f"\n2. Redeem the invite to subscribe\n"
            f"\n   POST /invite/{token}/redeem\n"
            f"   Header: Content-Type: application/json\n"
            f'   Body: {{"name": "{name}"}}\n'
            f"   → Returns human key + agent key\n"
            f"\n   Once subscribed, use dugg_feed and dugg_catchup to receive\n"
            f"   content from other members. Subscribe to more servers for\n"
            f"   more curated signal.\n"
            f"\nPartner guide (read before first submission):\n"
            f"  https://github.com/kadedworkin/dugg-fyi/blob/main/PARTNER_AGENT.md"
        )
